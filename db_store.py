...
def add_ride(patient_id: int, ride_date: str, distance_km: float, duration_min: int, rpe: Optional[int], notes: Optional[str]) -> None:
    conn = get_conn()
    cur = conn.cursor()
    cur.execute("""
...
def upsert_week_plan(patient_id: int, week_start: str, planned_km: Optional[float], planned_hours: Optional[float],
                     phase: Optional[str], notes: Optional[str]) -> None:
    conn = get_conn()
    cur = conn.cursor()
    cur.execute("""
