...
with tab2:
    ...
    token_row = db.get_strava_tokens(pid)
    ...
    if token_row is None:
        ...
        if refreshed:
            db.save_strava_tokens(pid, access_token, refresh_token, expires_at, athlete_id, str(scope))
        ...
        if st.button("Sync Strava rides"):
            ...
            while True:
                ...
                for a in acts:
                    ...
                    act_id = int(a["id"])
                    if db.is_activity_synced(pid, act_id):
                        continue
                    ...
                    db.mark_activity_synced(pid, act_id)
                    imported += 1
    ...
    plan_rows = db.fetch_week_plans(pid)
...
with tab4:
    ...
    profile = db.get_patient_profile(pid)
    ...
    if st.button("Create block + auto-generate weeks/sessions"):
        ...
        for wk in range(1, int(block_weeks) + 1):
            ...
            for lab in labels:
                sess_id = db.upsert_sc_session(week_id=week_id, session_label=lab, day_hint=None, notes=None)
                db.clear_sc_session_exercises(sess_id)
    ...
    latest = db.fetch_latest_sc_block(pid)
    if latest is None:
    ...
    detail = db.fetch_sc_block_detail(block_id)
        ...
        with st.expander(f"Week {wk_no} ({wk_start}) - Session {label} {'(DELOAD)' if is_deload else ''}", expanded=(wk_no == 1)):
            ...
            for ex in exs:
                ...
                if st.button("Save actual", key=f"save_actual_{row_id}"):
                    db.update_sc_session_exercise_actual(
                        row_id=row_id,
